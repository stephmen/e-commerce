...
const Mutations = {
  ...
  async requestReset(parent, args, ctx, info) {
    ...
    }/reset?resetToken=${resetToken}'>Clik Here to Reset</a>`)
    });
  ...
  async addToCart(parents, args, ctx, info) {
    ...
    if (!userId) {
      throw new Error("you must be singed in soon");
    }
    //2. Query user current car
    ...
    //3. Check if this current item is in the cart and if yes increment by one
    if (existingCartItem) {
    ...
    //4. If the item is not in the cart create a new fresh item in the cart.
  ...
  async removeFromCart(parent, args, ctx, info) {
    ...
    if (cartItem.user.id !== ctx.request.userId) {
      throw new Error("Cheatin huhhhhhh");
    }
  ...
  async createOrder(parent, args, ctx, info) {
    ...
    //6.Cleanup clean user cart delete cart Items
